                    ...
                    <tbody>
                                        @foreach($payments as $key => $payment)
                                            <tr data-entry-id="{{ $payment->id }}">
                                                ...
                                                <td>
                                                    @can('payment_show')
                                                        <a class="btn btn-xs btn-primary" href="{{route('payment.show',$payment->id)}}">
                                                            {{ trans('global.view') }}
                                                        </a>
                                                    @endcan

                                                    @can('payment_edit')
                                                        <a class="btn btn-xs btn-info" href="{{route('payment.edit',$payment->id)}}">
                                                            {{ trans('global.edit') }}
                                                        </a>
                                                    @endcan

                                                    @can('payment_delete')
                                                        <form action="{{ route('payment.destroy', $payment->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
